...
export class UsedCarService extends CrudService<typeof x> {
  ...
  async getnewmodels(brands: string[]) {
    ...
    for (let model of modelsSet) {
      ...
      newModel.count = (
        await this.repository.findAll({ modelGroup: model, status: 'online' })
      ).count;
  ...
  async findForSugerencias(
    ...
    if (onlybrand === true) {
      lista.items = (
        await this.repository.findAll({ brand: query.brand, status: 'online' })
      ).items;
    } else {
      lista.items = (
  ...
  async carverification(car) {
    ...
    if (car.transmision === '' || car.transmision === null) {
      return 'offline';
    }
  ...
  async zadCarCatalogue(agency, token) {
    let angeci: any = await this.httpService
      .get<{ success: boolean; message: string; data: SADUsedCar[] }>(
            ...
            if (sc.isAvailable === 'S' && sc.isReserved === 'N') {
              ...
              if (sc.chassisType === 'S U V' || sc.chassisType === 'SUV') {
                MetaDescription =
                  ...
                  ' ' +
                  sc.model.trim() +
                  ' Seminueva en línea, y te la llevamos a cualquier parte de México. 20 años de experiencia nos avalan. ¡Estrena tu auto ya!';
                h1 =
                  'Camioneta Seminueva ' +
                  sc.brand.trim() +
                  ' ' +
                ...
                MetaDescription =
                  ...
                  ' ' +
                  sc.model.trim() +
                  ' Seminuevo en línea, y te lo llevamos a cualquier parte de México. 20 años de experiencia nos avalan. ¡Estrena tu auto ya!';
                h1 =
                  'Auto Seminuevo ' +
                  sc.brand.trim() +
                  ' ' +
              ...
              let usedCar: UsedCar = {
                ...
                metaTitulo:
                  ...
                  ' ' +
                  sc.year.trim() +
                  ' Seminuevo en Línea | Estrena tu Auto',
