...
export class NewCarHelps {
  static groupCarsByHash(cars: NewCar[]): NewCar[] {
    ...
    let uniqueHashes = 0;
    for (let car of cars) {
      const params = `${car.brand}${
        car.model
      }${car.series.toString()}${car.agencyCity.toString()}${car.year.toString()}${car.transmision.toString()}${car.price.toString()}${
        car.colours
      }${car.agencyId}`;
  ...
  static getBaseColour(colour: string) {
    ...
    const foundColour = colourTab.find(
      (colourObject: { name: string; hex: string }) =>
        colourObject.name === baseColour,
    );
